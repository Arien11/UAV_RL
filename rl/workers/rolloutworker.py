...
class RolloutWorker:
    def __init__(self, env_fn, policy_template, critic_template, seed=None, worker_id=0):
        ...
        Args:
            env_fn: 用于生成环境的工具函数(called once)
    ...
    def sample(self, gamma, max_steps, max_traj_len, deterministic=False):
        ...
        memory = Buffer(self.state_dim, self.action_dim, gamma=gamma, size=max_steps)
        completed_ep_lens = []
        ...
        try:
            ...
            while len(memory) < max_steps:
                ...
                if episode_ended:
                    ...
                    # Bootstrap with value estimate if truncated, 0 if truly done 采用critic网络来估计截止状态(自举量)
