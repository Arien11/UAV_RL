...
class SmoothnessLoss(nn.Module):
    ...
    def forward(self, Df, Dp):
        ...
        """
        RJ = self._RJ.unsqueeze(0).expand(Dp.shape[0], -1, -1)  # (batch_size, 6, 6)
        RA = self._RA.unsqueeze(0).expand(Dp.shape[0], -1, -1)  # (batch_size, 6, 6)
        D_all = torch.cat([Df, Dp], dim=2)  # dx, dy, dz will be rows
